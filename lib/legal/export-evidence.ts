...
export async function exportEvidencePack(organisationId: string): Promise<EvidencePack> {
  ...
  if (orgError) console.error('[exportEvidencePack] organisation query error:', orgError)
  ...
  if (eventsError) console.error('[exportEvidencePack] events query error:', eventsError)
  ...
  if (aiSystemsError) console.error('[exportEvidencePack] ai_systems query error:', aiSystemsError)
  ...
  if (respError) {
    console.error('[exportEvidencePack] organisation_responsibilities query error:', respError)
  }
  ...
  const packCore: Omit<EvidencePack, 'integrity'> = {
    ...
    governance_snapshot: {
      ownership_model: 'single-owner enforced',
      audit_logging: 'enabled',
      organisation_events: (events as OrgEventRow[] | null | undefined ?? []).map((e) => ({
    ...
    legal_acceptance: (acceptanceRows as AcceptanceRow[] | null | undefined ?? []).map((row) => {
    ...
    ai_systems: (aiSystems as AISystemRow[] | null | undefined ?? []).map((s) => ({
      name: s.name,
      purpose: s.purpose,
      system_owner: s.system_owner ?? null,
      data_categories: Array.isArray(s.data_categories) ? (s.data_categories as string[]) : [],
      lifecycle_status: s.lifecycle_status,
      is_operational: Boolean(s.is_operational),
      last_updated: s.updated_at ? new Date(s.updated_at).toISOString() : generatedAt,
    })),

    responsibility_map: {
      declared_count: (responsibilities as ResponsibilityRow[] | null | undefined ?? []).length,
      records: (responsibilities as ResponsibilityRow[] | null | undefined ?? []).map((r) => ({
        role: r.role_label,
    ...
    ai_act_mapping: [
      ...
      },
      {
        ...
        evidence_refs: ['governance_snapshot.organisation_events', 'responsibility_map'],
      },
    ],
  }
